...
\chapterquote{Everything fails, all the time.}{Werner Vogels}
...
\begin{itemize}
    ...
    \item \textbf{Cost:} Cloud follows an \gls{OPEX} model, while on-premise is typically \gls{CAPEX} (see Section~\ref{sec:financial-management} for more details on these models).
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        node distance=2cm,
        pillar/.style={circle, draw=secondarycolor, thick, fill=secondarycolor!10, minimum size=3cm, align=center}
    ]
        \node[pillar] (metrics) {Metrics};
        \node[pillar, right=0.5cm of metrics] (logs) {Logs};
        \node[pillar, right=0.5cm of logs] (traces) {Traces};
    \end{tikzpicture}
    \caption{The Three Pillars of Observability}
    \label{fig:observability-pillars}
\end{figure}

\input{section4-technology/incident-management-and-reliability}
