...
\begin{figure}[h]
    ...
    \begin{tikzpicture}[node distance=2cm, auto, >=stealth, thick]
        \tikzset{block/.style={rectangle, draw, fill=secondarycolor!20, text width=3cm, text centered, rounded corners, minimum height=1.2cm}}
        \tikzset{line/.style={draw, ->}}
        
        \node [block] (job) {Job Definition};
        \node [block, right of=job, xshift=2.5cm] (interview) {Interviewing};
        \node [block, right of=interview, xshift=2.5cm] (onboarding) {Onboarding};
        
        \path [line] (job) -- (interview);
        \path [line] (interview) -- (onboarding);
    \end{tikzpicture}
    \caption{Hiring Process Flowchart}
    \label{fig:hiring_flowchart}
\end{figure}
...
\begin{figure}[h]
    \centering
    \begin{tikzpicture}[x=0.12cm, y=1cm, thick, >=stealth]
        % Horizontal line
        ...
        % Labels
        \node[above=0.5cm, text width=2.5cm, text centered] at (15,0) {Integration \& Learning};
        \node[above=0.5cm, text width=2.5cm, text centered] at (45,0) {Contribution \& Ownership};
        \node[above=0.5cm, text width=2.5cm, text centered] at (75,0) {Optimization \& Impact};
        
        % Arrows/Brackets
        \draw[<->] (0, -0.8) -- (30, -0.8) node[midway, below] {First 30};
        \draw[<->] (30, -0.8) -- (60, -0.8) node[midway, below] {First 60};
        \draw[<->] (60, -0.8) -- (90, -0.8) node[midway, below] {First 90};
    \end{tikzpicture}
    \caption{30/60/90 Day Onboarding Roadmap}
    \label{fig:onboarding_roadmap}
\end{figure}
